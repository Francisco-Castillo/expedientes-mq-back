...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<Object> handleException(final HttpMediaTypeException e) {
		...
		ErrorHTTPDTO error = ErrorHTTPDTO.builder().httpStatus(HttpStatus.UNSUPPORTED_MEDIA_TYPE)
				.errorCode(String.valueOf(HttpStatus.UNSUPPORTED_MEDIA_TYPE.value())).messages(List.of(e.getMessage()))
				.build();
	...
	public ResponseEntity<Object> handleRuntimeException(final MunicipalidadMQRuntimeException e) {
		...
		ErrorHTTPDTO error = ErrorHTTPDTO.builder().httpStatus(e.getHttpStatus()).errorCode(e.getErrorCode())
				.messages(List.of(e.getMessage())).build();
