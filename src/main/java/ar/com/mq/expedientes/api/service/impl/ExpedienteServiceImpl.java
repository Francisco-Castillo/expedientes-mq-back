...
public class ExpedienteServiceImpl implements ExpedienteService {
	...
	public ExpedienteServiceImpl(ExpedienteRepository expedienteRepository, ExpedienteMapper expedienteMapper,
			ParametroService parametroService, DocumentoMapper documentoMapper, PaseRepository paseRepository) {
	...
	public void save(ExpedienteDTO expedienteDTO) {
		...
		if (ObjectUtils.isNotEmpty(this.expedienteRepository.findByNumero(expedienteDTO.getNumero()))) {
			throw MunicipalidadMQRuntimeException
					.conflictException("Ya existe un expediente caratulado con el numero " + expedienteDTO.getNumero());
		}
	...
	@Override
	public WrapperData findAll(Integer page, Integer size, LocalDate startDate, LocalDate endDate, String type,
		...
		Page<Expediente> expedientePage = expedienteRepository.findAll(new Specification<Expediente>() {
			...
			public Predicate toPredicate(Root<Expediente> root, CriteriaQuery<?> cq, CriteriaBuilder criteriaBuilder) {
				...
				if (ObjectUtils.isNotEmpty(startDate)) {
					Predicate startDatePredicate = criteriaBuilder.greaterThanOrEqualTo(root.get("fechaCaratulacion"),
							startDate);
				...
				if (ObjectUtils.isNotEmpty(endDate)) {
					Predicate endDatePredicate = criteriaBuilder.lessThanOrEqualTo(root.get("fechaCaratulacion"),
							endDate);
				...
				if (StringUtils.isNotBlank(type)) {
					Predicate typePredicate = criteriaBuilder.like(criteriaBuilder.lower(root.get("tipo")),
							"%" + type.toLowerCase() + "%");
				...
				if (StringUtils.isNotBlank(status)) {
					Predicate statusPredicate = criteriaBuilder.like(criteriaBuilder.lower(root.get("estado")),
							"%" + status.toLowerCase() + "%");
				...
				if (ObjectUtils.isNotEmpty(caratuladorId)) {
					Predicate caratuladorIdPredicate = criteriaBuilder.equal(root.get("usuario").get("id"),
							caratuladorId);
				...
				if (StringUtils.isNotBlank(universalFilter)) {
					...
					Predicate universalPredicate = criteriaBuilder.like(criteriaBuilder.lower(root.get("numero")),
							"%" + universalFilter.toLowerCase() + "%");

					universalPredicate = criteriaBuilder.or(universalPredicate,
							criteriaBuilder.like(id, "%" + universalFilter.toLowerCase() + "%"));
					universalPredicate = criteriaBuilder.or(universalPredicate,
							criteriaBuilder.like(name, "%" + universalFilter.toLowerCase() + "%"));
					universalPredicate = criteriaBuilder.or(universalPredicate,
							criteriaBuilder.like(lastName, "%" + universalFilter.toLowerCase() + "%"));
					universalPredicate = criteriaBuilder.or(universalPredicate, criteriaBuilder.like(
							criteriaBuilder.lower(root.get("estado")), "%" + universalFilter.toLowerCase() + "%"));
					universalPredicate = criteriaBuilder.or(universalPredicate, criteriaBuilder.like(
							criteriaBuilder.lower(root.get("referencia")), "%" + universalFilter.toLowerCase() + "%"));
					universalPredicate = criteriaBuilder.or(universalPredicate, criteriaBuilder.like(
							criteriaBuilder.lower(root.get("descripcion")), "%" + universalFilter.toLowerCase() + "%"));
					universalPredicate = criteriaBuilder.or(universalPredicate, criteriaBuilder
							.like(criteriaBuilder.lower(root.get("tipo")), "%" + universalFilter.toLowerCase() + "%"));
	...
	public List<ExpedienteDTO> buscarTodosMisExpedientes(Long usuarioReceptorId) {
		...
		for (Pase item : pases) {
			lista.add(ExpedienteDTO.builder().id(item.getExpediente().getId())
					.iniciador(item.getExpediente().getIniciador()).numero(item.getExpediente().getNumero())
					...
					.codigoTramite(item.getExpediente().getCodigoTramite()).tipo(item.getExpediente().getTipo())
					.cantidadFojas(item.getExpediente().getCantidadFojas()).monto(item.getExpediente().getMonto())
					.estado(item.getExpediente().getEstado())
					.ultimaActualizacion(item.getExpediente().getUltimaActualizacion())
					.usuario(UsuarioDTO.builder().id(item.getUsuarioReceptor().getId())
							.nombre(item.getUsuarioReceptor().getNombre())
							.apellido(item.getUsuarioReceptor().getApellido()).build())
					.usuarioEmisor(UsuarioBaseDTO.builder().id(item.getUsuarioEmisor().getId())
							.nombre(item.getUsuarioEmisor().getNombre()).apellido(item.getUsuarioEmisor().getApellido())
							.build())
					.build());
		}
